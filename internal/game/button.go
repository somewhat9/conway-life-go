...
func NewButton(x, y float64, w, h int, label string, hov, click bool, clr, hovClr, fntClr color.Color, face text.Face) *Button {
...
func (b *Button) Draw(screen *ebiten.Image) {
	...
	tx, ty := text.Measure(b.label, b.face, 0)
	textX := b.x + (float64(b.w)-tx)/2
	textY := b.y + (float64(b.h)-ty)/2
	
	opts := &text.DrawOptions{}
	opts.GeoM.Translate(textX, textY)

	text.Draw(screen, b.label, b.face, opts)
}
